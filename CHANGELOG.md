...
- The method `UpdateItemsCategory` to update the category of multiple items at
  once.

### Updated

- The category view to include the sum of all debits and credits.

## [1.10.0] - 2023-11-02

### Added

- The `GetCategoryMonthTotals` method, which returns the total amount summed for
  each month for a category.
  - This is based on `entityUUID` for a specific user.
...
- The `GetCategoryItems` method to get all the items for a category.
...
- Update the types whose fields and nil values would be omitted by the `json`
  package. This is to ensure that the `json` package does not encode empty
  fields. This allows for partial updates of fields.
...
- The `IntegrationStatus` field to the `Account` type.
  - This field can now also by updated using the `UpdateAccount` method.
